...
**ALWAYS execute requests with direct `curl` commands.** Use the spec-extraction scripts (`api-specs-context.sh`, `extract-tags.js`, `extract-endpoint-detail.sh`) to discover endpoints, but make actual API calls with `curl`. `scripts/execute-request.sh` is a local development/testing helper only (use it manually with proper scopes/credentials); agents must use direct `curl` for production or automated requests.
